...
import { getAllBlogs } from "../../features/blogs/blogsApiSlice";
...
const Blog = () => {
  ...
  if (blogs.length > 0) {
    return (
      <>
        <Helmet>
          <meta property="og:url" content="https://react.kinsust.org/post" />
          <meta
            property="og:title"
                    ...
                    <PhotoProvider>
                      ...
                      >
                        <LazyLoadImage
                          alt={post?.title}
                          src={`${ApiURL}/public/images/posts/${post?.post_photo}`}
                          className=" cursor-pointer md:rounded-l-md rounded-t-md md:rounded-tr-none  object-cover w-full h-full "
                          effect="blur"
                          width={"100%"}
                          height={"100%"}
